...
stochastic_optimisation <- function(loss_f, linear_pred_f, 
  ...
  output_loss <- loss
  while (i < iters) {
    output_loss <- c(output_loss, loss)
    output_b <- c(output_b, random_m)
    temp_b <- runif(1, min = -2, max = 2)
    loss_temp <- loss_f(linear_pred_f(X, temp_b, 0), Y)
    if (loss_temp < loss) {
      random_m <- temp_b
      loss <- loss_temp
    }
  ...
  data.frame(iter = output_i, loss = output_loss, parameter = random_m)
}
...
run_many <- lapply(seq(n_runs), function(x) 
  stochastic_optimisation(sum_sq, linear_pred, X = X, Y = Y, 
                          lower_bound = -2, 
                          higher_bound = -2))
plot(NULL,xlim=c(0,250), ylim=c(0,10), type='n')
i <- 1
