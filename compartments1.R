...
# Let us try fitting a linear model again. 
# As we see, linear regression cannot now give a good fit.
model <- lm(conc ~ Time, data = d)
plot(conc ~ Time, data = d)
abline(model)

# We may initially be tempted to try to fit a quadratic model
d$Time2 <- d$Time ^2
model2 <- lm(conc ~ Time + Time2, data = d)

predict_x <- seq(0,25,0.1)
predict_y <- predict(model2, list(Time = predict_x, Time2 = predict_x ^ 2))
plot(conc ~ Time, data = d)
lines(predict_x, predict_y)

# Let's compare the models using the summary() function
summary(model)
summary(model2)
# model2 gives us a better "fit", with an R-squared of 0.23 compared to 0.039
# However the model still does not look good. Perhaps adding another 
# quadratic term helps? 

d$Time3 <- d$Time ^3
model3 <- lm(conc ~ Time + Time2 + Time3, data = d)

predict_x <- seq(0,25,0.1)
predict_y <- predict(model3, list(Time = predict_x, Time2 = predict_x ^ 2, 
                                 Time3 = predict_x ^ 3))
plot(conc ~ Time, data = d)
lines(predict_x, predict_y)
...
# We have a better "fit", with an R-squared of 0.51, but clearly this is a 
# ridiculous model. The region between time 15 and 20 predicts highly negative
# concentrations, which is not biologically plausible. This is a clear case of
# overfitting, and we need a different type of model. 
...
gradient_descent_analytical <- function(t, y, learning_rate, epochs, epsilon,  
  ...
  y_pred <- with(
    ...
    analytical_function(t, ka, kel, V, dose))
